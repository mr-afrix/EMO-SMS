            ...
            async with sess.post(
                ...
                if resp.status == 302 and "login" not in location.lower():
                    if location.startswith("http"):
                        ...
                        redirect_url = f"http://139.99.69.196{location}"
                    else:
                        redirect_url = f"{PANEL_BASE}/{location.lstrip('./')}"
                ...
                logger.warning(f"Login backoff: attempt {self._login_attempts}, next try in {backoff}s")
                return False
        ...
        try:
            ...
            if not panel._logged_in:
                ...
                if not ok:
                    ...
                    if panel._login_attempts == 1:
                        await notify_admins(
                            app,
                            f"ᴘᴀɴᴇʟ ʟᴏɢɪɴ ғᴀɪʟᴇᴅ — ʙᴀᴄᴋᴏғғ ᴀᴄᴛɪᴠᴇ\n"
            ...
            if keepalive_timer >= KEEPALIVE_INTERVAL:
                ...
                if idle_secs >= KEEPALIVE_INTERVAL:
                    ...
                    if not alive:
                        logger.warning("Keepalive failed — session may have expired, will detect on next fetch")
                keepalive_timer = 0
